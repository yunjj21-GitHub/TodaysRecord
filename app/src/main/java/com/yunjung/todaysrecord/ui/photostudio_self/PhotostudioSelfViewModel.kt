...
class PhotostudioSelfViewModel : ViewModel() {
    ...
    private fun getPsListOfUserAreaInBasicOrder(){
        viewModelScope.launch {
            val response = withContext(Dispatchers.IO){
                try {
                    RetrofitManager.service.getPhotoStudioByAreaAndType(area = userArea.value, type = "셀프")
                }
                catch (e : Throwable){
                    listOf()
                }
    ...
    private fun getPsListOfUserAreaInPopularityOrder(){
        viewModelScope.launch {
            val response = withContext(Dispatchers.IO){
                try {
                    RetrofitManager.service.getPsListOfUserAreaInPopularityOrder(area = userArea.value, type = "셀프")
                }
                catch (e : Throwable){
                    listOf()
                }
    ...
    private fun getPsListOfUserAreaInCostOrder(){
        viewModelScope.launch {
            val response = withContext(Dispatchers.IO){
                try {
                    RetrofitManager.service.getPsListOfUserAreaInCostOrder(area = userArea.value, type = "셀프")
                }
                catch (e : Throwable){
                    listOf()
                }
