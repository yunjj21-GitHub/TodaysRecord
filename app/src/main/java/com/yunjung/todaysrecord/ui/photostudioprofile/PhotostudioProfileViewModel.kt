...
class PhotostudioProfileViewModel : ViewModel() {
    ...
    private fun getPsListOfUserAreaInPopularityOrder(){
        viewModelScope.launch {
            val response = withContext(Dispatchers.IO){
                try {
                    RetrofitManager.service.getPsListOfUserAreaInPopularityOrder(area = userArea.value, type = "프로필사진")
                }
                catch (e : Throwable){
                    listOf()
                }
    ...
    private fun getPsListOfUserAreaInCostOrder(){
        viewModelScope.launch {
            val response = withContext(Dispatchers.IO){
                try {
                    RetrofitManager.service.getPsListOfUserAreaInCostOrder(area = userArea.value, type = "프로필사진")
                }
                catch (e : Throwable){
                    listOf()
                }
