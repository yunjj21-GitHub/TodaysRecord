...
class PhotostudioOtherViewModel : ViewModel() {
    ...
    private fun getPsListOfUserAreaInPopularityOrder(){
        viewModelScope.launch {
            val response = withContext(Dispatchers.IO){
                try {
                    RetrofitManager.service.getPsListOfUserAreaInPopularityOrder(area = userArea.value, type = "기타")
                }
                catch (e : Throwable){
                    listOf()
                }
    ...
    private fun getPsListOfUserAreaInCostOrder(){
        viewModelScope.launch {
            val response = withContext(Dispatchers.IO){
                try {
                    RetrofitManager.service.getPsListOfUserAreaInCostOrder(area = userArea.value, type = "기타")
                }
                catch (e : Throwable){
                    listOf()
                }
